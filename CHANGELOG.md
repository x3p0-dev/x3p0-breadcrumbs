...
- **`Crumb::getType()` has been renamed to `getSlug()` and is now abstract — another breaking change for custom `Crumb` classes.** There's no more class-name-derived fallback, so every concrete crumb must return its own stable slug.
