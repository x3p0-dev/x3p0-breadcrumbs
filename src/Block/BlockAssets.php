 ...
 * icon controls — including one per viewable post type and public taxonomy —
 * likewise come from {@see IconOptions}, which builds them on demand, so the
 * editor's controls and canvas preview crumbs stay in sync with the icons the
 * trail is really preset to rather than recreating the lookups client-side.
 * The {@see MetaKey} cases go over as well, since the canvas previews the open
 * post's own icon on the trail's last crumb.
 */
			...
			sprintf(
				...
				wp_json_encode(
					[
						...
						'postTypeIconPresetKeys' => $this->iconOptions->postTypeKeys(),
