 ...
 * WooCommerce). An extension wires itself into the plugin without any core edits
 * by using the same public seams available to third parties: it registers custom
 * query, assembler, or crumb types in `register()`, subscribes listeners to the
 * plugin's events via the `Subscriber` contract, and registers any WordPress
 * hooks in `boot()`. The `ExtensionServiceProvider` only wires up an extension
 * when `isSupported()` reports that its target platform is present, so nothing
 * runs when the platform is inactive. Concrete extensions are `final` and live
 * under their own sub-namespace; this class is the typehint the service provider
 * works against.
 */
abstract class Extension implements Bootable, Subscriber
{
	/**
