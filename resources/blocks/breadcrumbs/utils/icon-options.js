 ...
 * see `IconOptions::forBlock()` on the PHP side. PHP enumerates
 * everything, including one option per viewable post type and public taxonomy
 * (via `IconOptions`), so the editor never enumerates them
 ...
 * key namespacing (`IconPresetKey::postType()` and its siblings), and having
 ...
 * `IconPresetKey` enum on the PHP side. Everything else the panel renders is
 ...
 * crumbs, keyed by post type name — see `IconOptions::postTypeKeys()`. The
 ...
 * `IconPresetKey` owns the `post-type:{slug}` scheme, and rebuilding it here
