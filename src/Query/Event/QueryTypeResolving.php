 ...
 * `QueryKey` (typically a `QueryType` case), a string key for a custom one, or
...
final class QueryTypeResolving implements StoppableEvent
{
	 ...
	 * query type is mutable so listeners can override it; pass a `QueryKey`
	...
	public function __construct(
		...
		private QueryKey|string|null $queryType
	) {}

	/**
	 * Returns the query type resolved so far: a `QueryKey` (such as a
	 * `QueryType` case), a string key, or null when none matched.
	 */
	public function getQueryType(): QueryKey|string|null
	{
		return $this->queryType;
	}

	/**
	 * Overrides the query type to build for. Pass a `QueryKey` (such as a
	 * `QueryType` case), a string key for a custom type, or null to build
	 * no breadcrumbs.
	 */
	public function setQueryType(QueryKey|string|null $queryType): void
	{
